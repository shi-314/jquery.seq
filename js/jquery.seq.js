...
(function (mme2) {
	...
	$.fn.sequence = function (options) {
		...
		this.setData = function (data) {
			...
			for(var i = 0; i < elements.length; i++) {
				var e = elements[i];
				var x = parseInt(e.position.x);
				var y = parseInt(e.position.y);
				this.add(x, y, e.label);
			}

			for(var i = 0; i < connections.length; i++) {
				var c = connections[i];
				this.connect(c.element1, c.element2, c.element1Anchor, c.element2Anchor);
			}
	...
	mme2.SequenceEditor.prototype.getElementById = function (id) {
		...
		console.log('warning: no element found with idx '+id);
